...
import com.company.test1.application.handlers.SwitchPerspectiveHandler;
import com.company.test1.application.parts.editor.treeelements.TreeElement;
import com.company.test1.application.parts.editor.treeelements.TreeElementScenario;
import com.company.test1.application.states.AppState;
import com.company.test1.application.utils.EditorTreeLabelProvider;
import com.company.test1.application.utils.IconsMapping;
import com.company.test1.application.utils.MainWindowTitleUpdater;
...
public class TreePart {
	...
	private ToolItem schedulingPerspectiveItem;
	...
	public void postConstruct(Composite parent) {
		DataBinding.getInstance(eventBroker);
	...
	private void initializeToolBar(Composite parent) {
		...
		schedulingPerspectiveItem = ToolbarUtils.addCommandItem(toolBar, IconsMapping.getImage("/icons/planning.png"), messages.button_planning, 
				() -> SwitchPerspectiveHandler.trySwitchToPerspective(Perspective.SCHEDULING, parent.getShell(), app, partService, modelService, messageManager, messages, appData, appState
		));
		schedulingPerspectiveItem.setEnabled(false);
		schedulingPerspectiveItem.setText(messages.button_planning);
	}
	...
	private void initializeTreeViewer(Composite parent) {
		...
		treeViewer.setLabelProvider(new EditorTreeLabelProvider() {
			...
			public Image getIcon(Object object) {
				if (object instanceof TreeElement treeElement){
					return treeElement.getTreeElementType().getIcon();
				}
				return IconsMapping.getImage("icons/dummy.png");
			}
	...
	private void onNewFileLoaded() {
		...
		if (scenario != null) {
			...
			treeViewer.expandAll();
			schedulingPerspectiveItem.setEnabled(true);
		}
